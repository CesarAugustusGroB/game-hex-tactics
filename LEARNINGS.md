...
## Rigid-block march is all-or-nothing — fatal for large blobs that touch (`feature/systems`)

After the wide-front AI doctrine landed, two reports ("the AI never attacks with cavalry/skirmishers", "the AI never moves the right-flank units") turned out to be the *same* bug. The AI fields four lateral bands of 80+ units that pack tightly enough to **touch at their boundaries**. `march` is a rigid block: `tryRigidBlockStep` commits all-or-nothing, so if *any* unit's forward hex is occupied (by a neighbouring band's unit, off-grid, or terrain), the **entire** band is frozen for that tick. Each band had only 1–4 boundary units blocked by the adjacent band — enough to freeze all 85. Only the first band processed (clear path ahead) ever advanced; every other band sat in the deploy zone holding a `march` order it could never execute. The fix was to issue the AI's march orders with `looseFormation: true` (the per-unit advance mode): the blob flows forward around the few blocked boundary units instead of deadlocking on them. Rigid block is correct for a small, deliberately-aligned player formation; it's the wrong default for big AI masses that inevitably touch. Behavioural regression (`test-ai-advance.ts`) drives the real controller + `simulateTick` over the real deploy zone and asserts every band leaves the zone — it fails 1/4 under rigid, 4/4 under loose.

Two debugging traps from the same session, both about **modelling the environment faithfully before theorising**: (1) The isolated controller deployed all four types fine — the bug only appeared with `simulateTick` in the loop (it's about *movement*, which a deploy-only harness can't show). Reproduce at the layer the symptom lives in. (2) I first "fixed" a CP-starvation story using a regen of 0.1 CP/tick — but the real rate is `CP_REGEN_PER_TICK_STEP (0.1) * regenN (10) = 1.0`/tick, 10× higher. The wrong constant produced a plausible-but-wrong root cause (march-order starvation) and a fix (round-robin scheduling) that addressed a condition the game rarely hits. The scheduling is kept as a cheap fairness guard, but the *actual* freeze was rigid-block. Read the constant's derivation, don't eyeball the JSON field.

## `march` moves along `heading`, not toward `attackTarget` (`feature/systems`)

Building the AI's reserve raid-defence (the reserve band peels off the centre push to intercept enemies breaching our *own* deploy zone), the obvious first cut — `issueOrder(reserve, { mode: 'march', attackTarget: raidThreatHex })` — sent the band the wrong way. In `simulate.ts`, `march` steps every unit by `HexUtils.directions[order.heading]` — one of the **6 fixed axial directions**, recomputed never. `attackTarget` does **not** steer movement; it's only fed to `projectedRanking` to order units front-to-back (so a lead unit vacates its hex before the follower steps in). So a march order with the right target but a forward `heading` walks the reserve *toward the centre*, away from the raid behind it. The defence has to set `heading = headingToward(groupCentroid, raidThreatHex)` — pick the one of six directions whose single step lands closest to the target — and set `attackTarget` too (so the rear ranks, nearest the breach, lead). Consequence worth internalising: `march` is a straight-line shove in a compass direction, not pathfinding. It can't curve around an obstacle or home on a moving target; only `unleash` does per-unit target-seeking. Any future "send this group to an arbitrary hex" order must compute a heading, not just a target — or use `unleash`.

### `Math.max(A, A - B)` collapses to `A` for `B ≥ 0` — a half-blind danger signal

The AI's counterattack threshold blends "enemy close to winning" with "we're behind on VP". The first cut wrote `vpDanger = Math.max(enemyScore, enemyScore - myScore) / POINTS_TO_WIN` — meaning to say "panic if the enemy is near 200 OR if they're far ahead of us". But for any `myScore ≥ 0`, `enemyScore - myScore ≤ enemyScore`, so `Math.max` always returns `enemyScore` and **`myScore` never enters the calculation**: building our own lead did nothing to lower the alarm. A parallel code-review agent flagged it at 95% confidence; the unit test passed only because it exercised the `myScore = 0` case, where the bug is invisible (both terms equal). The fix is an actual blend that uses both scores: `0.5 * (enemyScore / WIN) + 0.5 * (max(0, enemyScore - myScore) / WIN)`. Lesson: `max(x, x - k)` for non-negative `k` is a no-op that *looks* like it combines two signals — whenever a "max of two readings" has one reading provably dominating the other, you've written a constant. And a test that only covers the symmetric/zero case will green-light it.

A second, smaller point from the same change: the perception layer (`src/battle/ai/perception.ts`, the Tier 0 "eyes") was added as a **pure function nothing consumed yet**, then wired in by Tier 1. That sequencing is deliberate and worth repeating for AI work — `perceive(state)` is independently testable (`test-ai-perception.ts`, no sim, no controller), so the reactive behaviour built on top (`test-ai-defense.ts`) only has to test the *decision*, not re-derive the world facts. The authored-rules engine extended cleanly: new `RuleCtx` perception fields are **optional**, so the deploy/amass phase (which omits them) is unchanged — a `when: { threatened: true }` rule simply never matches when `ctx.threatened` is `undefined`, because `matches` treats `true !== undefined` as a miss. New AI facts can be added to the ruleset without touching the phases that don't populate them.

## Tuning AI difficulty by self-play is a fitness landscape, not a dial (`feature/ai-enemy`)

An hourly self-play loop (head-to-head `runMatch`, side-bias-cancelled win-rate at ≥30 reps/side) tried to ratchet the `test` difficulty above `hard` by simple parameter edits to `ai.json`. The mechanics that fell out are worth keeping, because they invert the intuition that "more tactics = harder AI":

- **The win condition makes camping the flag dominant, so two pure campers DRAW, they don't fight.** Both `normal` and `hard` were `defend`+`frontLines` configs that never leave to score; mirror matches run to the 2000-tick cap with no winner. In a side-cancelled win-rate that reads as **0%, not 50%** — a column of zeroes means "always draws", not "always loses". Any fitness function over camp-vs-camp has to treat the draw explicitly or it looks like total failure.
- **Against a camper the only thing that wins is mass: raising `forceScale` overruns the line.** But force **saturates at ~1.2** — beyond that the whole `INITIAL_ROSTER` is already deployed, so higher values are no-ops (and the noisy region around the saturation knee throws spurious 100%-loss cells at low reps; always re-confirm a surprising cell at 30+ reps before believing it).
- **Tactical capabilities are handicaps, not skill** (the `ai.ts` comment already says this; self-play confirmed it hard). Adding `focusFire`/`charge`/`unleash`/`repel`/`earlyLaunch` to a winning rush is uniformly ~50/50 (neutral), and `defend` actively *loses* once you're past the force ceiling — it pulls the army off the score path. The strongest reachable config kept *shedding* abilities down to `raid`-only.
- **`reactionTicks` is non-transitive (rock-paper-scissors), not monotonic.** Faster reaction beat slower in one matchup and lost to it in the next; "ratchets" along this axis are real 100% wins that encode no actual strength gain. A loop that only edits one such knob will report endless progress while going in circles.

The practical upshot: the storable simple-parameter space (force + reaction + capability set + deploy flags, doctrine fixed) is small and **exhausts to a genuine local maximum** — a point where *every* neighbouring config ties or loses. Real further difficulty needs a new axis. We added one by moving **doctrine into the difficulty entry** (`DifficultyConfig.doctrine?`, applied by `profileFromDifficulty` and on difficulty-select in `setTeamAi`, overridable by the player): switching `balanced`→`aggressive` was the only thing that beat the local-max `raid`-only camp. Keep the override precedence in mind — the per-team profile doctrine still wins, so the difficulty's doctrine is only a *default* applied at selection time, not a hard lock.

## A test that drives a difficulty *by name* rots when that difficulty's deploy flags drift (`feature/ai-enemy`)

A maintenance loop found six red AI regression files (`counter`, `advance`, `front`, `groups`, `rotation`, plus config/profile assertions) — every one a **stale test, not a controller bug**. The pattern: each test picked a difficulty by name (`makeAiController('blue', 'balanced', 'hard')`) to exercise a *mechanism* — the danger-based early launch (`launchShare`), the 4-band parallel front, the round-robin amass that keeps cavalry/skirmisher from starving. Those mechanisms all live in the **parallel-front** code path. But the `hard`/`normal` difficulties had since gained `frontLines: true`, and the frontLines/serialWaves path gates launch on the full `bandCap` and routes *around* `launchShare` entirely (`controller.ts`: `const full = serialFill ? bandCap : launchShare`). So the test was driving the one config whose deploy flags bypass the very thing it verifies, and it silently went red as the difficulty was retuned. The fix is never to touch the controller (that would be unmeasured AI tuning — see the fitness-landscape note): pin an **explicit `makeAiControllerProfile` with the deploy flags the mechanism needs** (`frontLines/horizontalFront/serialWaves: false`), so the test is immune to difficulty drift. Two corollaries worth internalising. (1) A config-validation test must assert against the *source* (`resolved.capabilities === AI.difficulties.test.capabilities`) or sanity bounds, never a hardcoded value that the authored config can retune out from under it — and a *balance* assertion (`hard.cpBudgetFrac > easy`) has no business in a config test once difficulty moved to a capability ladder with flat force/CP. (2) Beware seed positions colliding with meaningful map cells: two `groups` failures were the seed sitting on the capture centre `(0,0)`, where the hold-centre rule rewrites the order to `hold` — which *unseals* the "sealed" group and masks the can't-amass→march fall-through. The controller was right; the synthetic zone happened to put its corner on the objective.

A process note: when a `for f in scripts/test-*.ts` sweep reports the pass count, read the `N/M passed` line from the test's own output — do **not** pipe to `tail` and check `$?`, you'll capture `tail`'s exit code (always 0) and miss every failure.

## Compare sprite silhouettes with a *scale-independent* IoU — fixed-size IoU conflates scale with shape (`feature/ai-enemy`)

Re-normalizing the faction sprites to a uniform per-type visible size (cavalry→144, infantry→132) needed a headless safety gate: does the re-normalized sprite match the current one, or did the pipeline (chroma-key → `normalize-units.py`) mangle / mis-orient it? The first gate — alpha-silhouette IoU at the fixed 160×160 canvas — flagged 6 of 9 cavalry as "low" (0.49–0.80) and nearly made me defer them. That was wrong: the **whole point** of the operation is to change the visible scale, so a fixed-size IoU is dominated by exactly the difference we're introducing. The decisive test is a **scale-independent shape IoU**: crop each alpha mask to its figure bounding box, resize one to the other, *then* compare. That read 0.85–0.99 for all 9 — same unit, same orientation, only the scale differs (the intended fix). Lesson: when you're deliberately changing one property (scale), your verification metric must be invariant to that property, or it just measures your own edit. A second, genuinely-deferred case sharpened the line between "safe rescale" and "needs human eyes": for spear/overhead infantry the scale-free IoU stayed low (0.59–0.72) because the framing of an extended weapon legitimately differs by normalization mode (`--body-scale` scales the dense body and lets the spear clip, vs full-bbox which shrinks the whole figure) — and the current sprites' provenance was mixed, so there was no single ground truth to match. That's a visual judgement a headless gate can't make; it went to the user, who chose `--body-scale`. The gate's job is to *partition* into "provably the same up to the intended change" (apply) and "genuinely ambiguous" (escalate), not to force every case through.
