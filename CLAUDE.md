...
1. **`terrainGfx` (Pass 1, per-hex Graphics)** — top polygon fill (flat colour or `Texture` for `RIVER`/`SAND`/`SEA`/`DEEP_SEA`) plus shaded S/SE/SW side walls. Walls are drawn **only for the non-textured biomes** `SAND` and `ROCKY` (`isTexturedBiome` in `drawTerrain.ts`); `RIVER`/`SEA`/`DEEP_SEA` are textured and get their cliff faces from the filtered water parents and dedicated cliff passes, and the other textured land biomes get theirs from Pass 2. Hexes iterate in ascending `TERRAINS[type].height` so taller terrain renders after — and thus over — its shorter neighbours.
