  ...
  return (state: AiTickState): void => {
    ...
    for (const grp of marchOrder) {
      ...
      let action = evaluateRules(AI.rules, {
        ...
        // For frontLines (mixed-type) groups this is the doctrine SLOT type, not the group's
        // actual mix — fine while test lacks charge/unleash (those downgrade to march anyway). If a
        // frontLines tier ever gains them, derive groupType from the group's dominant unit type.
