...
import type { Unit, GroupOrder, Team, UnitType } from '../src/battle/simulate';
...
function bisect(reps: number) {
  ...
  for (const name of ['hard', ...Object.keys(variants), 'test']) {
    ...
    for (let i = 0; i < reps; i++) {
      const w = runMatch({ doctrine: 'balanced', difficulty: name as Difficulty }, { doctrine: 'balanced', difficulty: name as Difficulty }, { reverse: REVERSE_TICK_ORDER }).winner;
